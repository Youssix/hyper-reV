...
function formatResponse(res: { ok: boolean; data?: any; error?: string }): string {
  if (!res.ok) {
...
server.tool(
  "get_status",
  "Get HyperREV Inspector status: hypervisor connection state and attached process info",
  {},
  async () => {
...
server.tool(
  "list_processes",
  "List all running processes visible to the hypervisor. Returns PID, name, CR3, base address.",
  {},
  async () => {
    const res = await client.listProcesses();
    if (res.ok && Array.isArray(res.data)) {
      const lines = res.data.map(
        (p: any) => `${String(p.pid).padStart(6)}  ${p.name.padEnd(30)}  Base: ${p.base}  CR3: ${p.cr3}`
      );
      return {
...
server.tool(
  "attach_process",
  "Attach to a process by name (substring match) or PID. Required before memory operations.",
  { process: z.string().describe("Process name (substring) or PID number") },
  async ({ process }) => {
...
server.tool(
  "read_memory",
  "Read raw memory from the attached process. Returns hex dump with ASCII. Use for examining data structures, strings, vtables.",
  {
...
server.tool(
  "write_memory",
  "Write raw bytes to memory in the attached process. Bytes as hex string (e.g. '90909090' for NOPs).",
  {
...
server.tool(
  "read_value",
  "Read a typed value from memory. Types: int8, uint8, int16, uint16, int32, uint32, int, float, double, uint64, ptr64.",
  {
    address: z.string().describe("Virtual address in hex"),
...
server.tool(
  "disassemble",
  "Disassemble instructions at an address. Returns IDA-style listing with module+offset, bytes, and Intel syntax.",
  {
    address: z.string().describe("Virtual address in hex"),
...
server.tool(
  "resolve_address",
  "Resolve a virtual address to module+offset and export name (if at an export entry point).",
  { address: z.string().describe("Virtual address in hex") },
  async ({ address }) => {
    const res = await client.resolveAddress(address);
    if (res.ok && res.data) {
      let text = `${res.data.address} => ${res.data.module_address}`;
      if (res.data.export_name) text += ` (${res.data.export_name})`;
...
server.tool(
  "list_modules",
  "List all loaded modules in the attached process with base address and size.",
  {},
  async () => {
    const res = await client.listModules();
    if (res.ok && Array.isArray(res.data)) {
      const lines = res.data.map(
        (m: any) => `${m.base}  ${m.size.padStart(10)}  ${m.name}`
      );
      return {
...
server.tool(
  "find_export",
  "Search for exported functions across all modules (or a specific module). Substring match on export name.",
  {
  ...
  async ({ name, module }) => {
    ...
    if (res.ok && Array.isArray(res.data)) {
      const lines = res.data.map(
        (e: any) => `${e.address}  ${e.module}!${e.name}`
      );
      return {
...
server.tool(
  "start_code_filter",
  "Start monitoring a memory address via EPT page monitoring. Collects all instructions (RIPs) that read/write/execute the address. Like CE's 'Find what accesses this address'.",
  { address: z.string().describe("Virtual address to monitor in hex") },
  async ({ address }) => {
...
server.tool(
  "stop_code_filter",
  "Stop the current CodeFilter monitoring session.",
  {},
  async () => {
...
server.tool(
  "get_code_filter_results",
  "Get CodeFilter results: all unique instruction addresses that accessed the monitored address, with instruction text, access type (Read/Write/Execute), and hit count.",
  {},
  async () => {
...
server.tool(
  "add_breakpoint",
  "Add an EPT breakpoint on a virtual address. Monitors the physical page — logs all trap frames (registers + stack) when any code on that page executes.",
  {
    address: z.string().describe("Virtual address in hex"),
...
server.tool(
  "remove_breakpoint",
  "Remove an EPT breakpoint by virtual address.",
  { address: z.string().describe("Virtual address of breakpoint to remove") },
  async ({ address }) => {
...
server.tool(
  "list_breakpoints",
  "List all EPT breakpoints with their status, hit counts, and page GPA.",
  {},
  async () => {
    ...
    if (res.ok && Array.isArray(res.data)) {
      ...
      const lines = res.data.map(
        (bp: any) =>
          `${bp.active ? "[ON] " : "[OFF]"}  ${bp.address}  Page: ${bp.page_gpa}  Hits: ${bp.hit_count}  ${bp.label}`
      );
      return {
...
server.tool(
  "get_breakpoint_logs",
  "Get trap frame logs from EPT breakpoints. Each log entry contains full register state (RAX-R15, RIP, RSP, CR3) and 5 stack values. Most recent entries first.",
  {
...
server.tool(
  "ff_load_functions",
  "Load function addresses from a module using the specified source. Sources: 'combined' (pdata + CALL scan merged, maximum coverage — RECOMMENDED), 'pdata' (PE exception directory, fast), 'call_scan' (disassemble .text for CALL targets, thorough), 'trace' (EPT execute monitoring, finds everything that runs).",
  {
...
server.tool(
  "ff_start_monitoring",
  "Start EPT monitoring on all loaded function pages. After starting, interact with the target to trigger functions, then use ff_keep_executed or ff_remove_executed to filter.",
  {},
  async () => {
...
server.tool(
  "ff_stop_monitoring",
  "Stop EPT monitoring (or trace collection). Preserves the current function list and executed flags.",
  {},
  async () => {
...
server.tool(
  "ff_keep_executed",
  "Filter: keep only functions that were executed since last reset. Removes all non-executed functions. Resets executed flags afterwards.",
  {},
  async () => {
...
server.tool(
  "ff_remove_executed",
  "Filter: remove functions that were executed since last reset. Keeps only non-executed functions. Resets executed flags afterwards.",
  {},
  async () => {
...
server.tool(
  "ff_status",
  "Get current Function Filter state: phase (idle/loading/loaded/monitoring), source, module, function count, executed count, page count.",
  {},
  async () => {
...
server.tool(
  "ff_get_functions",
  "Get the function list with executed status. Returns address, name, and whether each function was executed.",
  {
