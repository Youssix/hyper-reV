...
namespace anim
{
	// current time in seconds (wraps ImGui::GetTime)
	...
	// 0-1 fade in from start_time over duration seconds
	...
	// slide offset: returns pixels to offset (starts at offset_px, ends at 0)
	...
	// staggered slide offset for item at index
	...
	// shimmer: returns x position (0 to width) for a sweep effect
	inline float shimmer_x(float width, float speed = 0.8f, float start_time = 0.0f)
	{
		float t = fmodf((time() - start_time) * speed, 1.0f);
		return t * (width + 60.0f) - 30.0f; // 30px wide shimmer band, sweeps across
	}
